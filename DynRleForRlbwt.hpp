...
namespace itmmti
{
    ...
    public:
        ...
        BtmNodeM_StepCode(
            uint16_t initStccCapacity = 0) : stcc_(),
                                             label_(0),
                                             parent_(nullptr),
                                             stccCapacity_(0),
                                             stccSize_(0),
        ...
        uint64_t calcSumOfWeight(
            ...
            const uint8_t childIdx_end) const noexcept
        {
        ...
        void setParentRef(
            ...
            uint8_t newIdxInSibling) noexcept
        {
         ...
         */
        void setNumChildren(
            const uint8_t newSize) noexcept
        {
    ...
    public:
         ...
         */
        uint64_t read(
            const uint64_t idx) const noexcept
        {
    ...
    public:
        DynRleForRlbwt(
            ...
            const uint64_t initSampleUb = 0) : srootM_(),
                                               idxM2S_(),
                                               btmMInfo_(),
                                               srootA_(),
                                               idxS2M_(),
        ...
        uint64_t calcSumOfWeightOfBtmS(
            const uint64_t btmS) const noexcept
        {
            assert(isValidBtmS(btmS));

            uint64_t sum = 0;
            for (uint64_t i = 0; i < getNumChildrenFromBtmS(btmS); ++i)
            { // use OpenMP?
        ...
        uint64_t calcSumOfWeightOfBtmS(
            ...
            uint8_t childIdx_end) const noexcept
        {
            assert(isValidBtmS(btmS));
            assert(childIdx_beg <= childIdx_end);
    ...
    public:
        ...
        const BTreeNodeT *searchCharA(
            const uint64_t ch) const noexcept
        {
            assert(isReady());
        ...
        uint64_t searchPosS(
            uint64_t &pos,
            const BTreeNodeT *rootS) const noexcept
        {
            assert(isReady());
            assert(rootS); // rootS should be valid node
        ...
        uint64_t calcPredIdxSFromIdxM(
            ...
            const uint64_t ch,
            const uint64_t idxM) const noexcept
        {
            // {//debug
            ...
            if (btmM)
            { // If btmM is not 0 (0 means btmM is the first btm in the mixed tree).
                while (i < kBtmBM && getCharFromIdxM(btmM * kBtmBM + i) != ch)
                { // "i < kBtmB" breaks when "i becomes below 0"
                    --i;
                }
                if (i < kBtmBM)
                {
            ...
            { // btmM == 0: dummy idx (== 0) should be ignored.
    ...
    public:
        ...
        ) const noexcept
        {
            ...
            const auto prevBtmM = reinterpret_cast<uint64_t>(getParentFromBtmM(btmM)->getPrevBtm(getIdxInSiblingFromBtmM(btmM)));
        ...
        ) const noexcept
        {
            ...
            const auto nextBtmM = reinterpret_cast<uint64_t>(getParentFromBtmM(btmM)->getNextBtm_DirectJump(getIdxInSiblingFromBtmM(btmM)));
        ...
        ) const noexcept
        {
            ...
            const auto prevBtmS = reinterpret_cast<uint64_t>(getParentFromBtmS(btmS)->getPrevBtm(getIdxInSiblingFromBtmS(btmS)));
        ...
        ) const noexcept
        {
            ...
            const auto nextBtmS = reinterpret_cast<uint64_t>(getParentFromBtmS(btmS)->getNextBtm_DirectJump(getIdxInSiblingFromBtmS(btmS)));
        ...
        ) const noexcept
        {
            ...
            const auto prevBtmM = reinterpret_cast<uint64_t>(getParentFromBtmM(idxM / kBtmBM)->getPrevBtm(getIdxInSiblingFromBtmM(idxM / kBtmBM)));
            if (prevBtmM != BTreeNodeT::NOTFOUND)
            {
        ...
        ) const noexcept
        {
            ...
            const auto nextBtmM = reinterpret_cast<uint64_t>(getParentFromBtmM(idxM / kBtmBM)->getNextBtm_DirectJump(getIdxInSiblingFromBtmM(idxM / kBtmBM)));
            if (nextBtmM != BTreeNodeT::NOTFOUND)
            {
        ...
        uint64_t getPrevIdxS(const uint64_t idxS) const noexcept
        {
            ...
            const auto prevBtmS = reinterpret_cast<uint64_t>(getParentFromBtmS(idxS / kBtmBS)->getPrevBtm(getIdxInSiblingFromBtmS(idxS / kBtmBS)));
            if (prevBtmS != BTreeNodeT::NOTFOUND)
            {
        ...
        uint64_t getNextIdxS(const uint64_t idxS) const noexcept
        {
            ...
            const auto nextBtmS = reinterpret_cast<uint64_t>(getParentFromBtmS(idxS / kBtmBS)->getNextBtm_DirectJump(getIdxInSiblingFromBtmS(idxS / kBtmBS)));
            if (nextBtmS != BTreeNodeT::NOTFOUND)
            {
    ...
    private:
            ...
            const uint64_t btmM) noexcept
        {
            ...
            while (true)
            {
                while (prev != BTreeNodeT::NOTFOUND && (getLabelFromBtmM(prev) >> l) == base)
                { // expand backward
                    ++num;
                    tmpBtmM = prev;
                    prev = getPrevBtmM(prev);
                }
                while (next != BTreeNodeT::NOTFOUND && (getLabelFromBtmM(next) >> l) == base)
                { // expand forward
                    ++num;
        ...
        void changePSumFromParentM(
            const uint64_t btmM,
            const int64_t change) const noexcept
        {
            // {//debug
            ...
            ) noexcept
        {
            ...
            if (isNewElemInL)
            {
                ...
                numL += numChild_ins;
                if (numL < numL_new)
                { // Still need to move elements to left after inserting srcWCodes
            ...
            if (numR_old != childIdx + numChild_del)
            { // There are remaining children in tail.
                if (numR_old != numR_new)
                { // Need shift wCodes of "this" node.
            ...
            ) noexcept
        {
            ...
            if (numR_old)
            { // shift wCodes of R to make space
        ...
        void writeNewElemInOneBtmM(
            ...
            const uint8_t numChild_ins) noexcept
        {
            // {//debug
            ...
            ) noexcept
        {
            ...
            if (num <= static_cast<uint16_t>(kBtmBM))
            { // Easy case: This node can accommodate inserting elements.
            ...
            if (idxInSib + 1 < parent->getNumChildren())
            { // Check next sibling.
        ...
        uint64_t insertRunWithSplitM(
            const uint64_t idxM,
            const uint64_t splitPos) noexcept
        {
            // {//debug
        ...
        uint64_t overflowToLeftS(
            const uint64_t lIdxBase,
            const uint64_t rIdxBase,
            const uint8_t childIdx) noexcept
        {
            // {//debug
    ...
    public:
        ...
        void setSample(
            const uint64_t idxM,
            const uint64_t newSample) noexcept
        {
            assert(isValidIdxM(idxM));
        ...
        )
        {
            ...
            else if (pos == 0)
            {
                ...
                if (idxM > 0 && ch == getCharFromIdxM(idxM))
                { // Check if 'ch' can be merged with the previous run.
                    pos = getWeightFromIdxM(idxM);
                    changeWeight(idxM, 1);
                }
                else
            ...
            { // Current run is split with fstHalf of weight 'pos'.
        ...
        uint64_t optInsert(
            ...
            const CharT ch)
        {
            // first: Find if the position of sap_s − 1 is exactly ch
            ...
            // second: Determine whether the run at this position sap_s exceeds sap_e
            pos = sap_s;
            idxM = searchPosM(pos);
            auto weight = getWeightFromIdxM(idxM);
